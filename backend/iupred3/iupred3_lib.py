...
def iupred(seq, mode='long', smoothing='medium'):
    if mode == "short":
        lc = 1
        uc = 25
        wc = 10
    ...
    if smoothing == 'medium':  # Medium
        iupred_score = list(savgol_filter(iupred_score, 19, 5))
    elif smoothing == 'strong':  # Strong
        iupred_score = smooth(iupred_score, 11)
...
def anchor2(seq):
    ...
    iupred_scores = iupred(seq, smoothing='None')[0]
